...
import { getLocalOrderOfPlayers } from "../../Util";
...
function Game(props) {
  ...
  useEffect(() => {
    ...
    function updateLocalState(boardState) {
      startingPlayer.current = boardState.startingPlayer;
      console.log(
        props.playerId + ":" + JSON.parse(playersOrderLocal.current[0].hand)
      );
      setP1(playersOrderLocal.current[0]);
      setP2(playersOrderLocal.current[1]);
      //   setP3(playersOrderLocal[2]);
      //   setP4(playersOrderLocal[3]);
      setTable(boardState.table ? boardState.table : []);
    }
  ...
  return (
    <div>
      ...
      <Player
        isPlayer={true}
        hand={player1.hand ? JSON.parse(player1.hand) : player1}
